...
bool ApplicationClass::Initialize(int screenWidth, int screenHeight, HWND hwnd) {
	...
	if (!m_Model[0].Initialize(m_Direct3D->GetDevice(), m_Direct3D->GetDeviceContext(), modelFilename, textureFilename, normalMapFilename)) {
		MessageBox(hwnd, L"Could not initialize the model object.", L"Error", MB_OK);
		return false;
	}
	strcpy_s(modelFilename, "../DX11/Assets/Models/cube.txt");
	if (!m_Model[1].Initialize(m_Direct3D->GetDevice(), m_Direct3D->GetDeviceContext(), modelFilename, textureFilename, normalMapFilename)) {
		MessageBox(hwnd, L"Could not initialize the model object.", L"Error", MB_OK);
		return false;
	}
	strcpy_s(modelFilename, "../DX11/Assets/Models/sphere.txt");
	if (!m_Model[2].Initialize(m_Direct3D->GetDevice(), m_Direct3D->GetDeviceContext(), modelFilename, textureFilename, normalMapFilename)) {
		MessageBox(hwnd, L"Could not initialize the model object.", L"Error", MB_OK);
		return false;
	}
	...
	if (!m_NormalMapShader->Initialize(m_Direct3D->GetDevice(), hwnd))
	{
...
bool ApplicationClass::Render()
{
	...
	for (int i = 0; i < 3; i++) {
		if (i == 0) {
			worldMatrix = XMMatrixScaling(0.3, 0.3, 0.3);
		}
		else
			worldMatrix = XMMatrixTranslation(i * 3, 1, 0);
		m_Model[i].Render(m_Direct3D->GetDeviceContext());
		if (!m_NormalMapShader->Render(m_Direct3D->GetDeviceContext(), m_Model[i].GetIndexCount(), worldMatrix, viewMatrix, projectionMatrix,
			m_Model[i].GetTexture(0), m_Model[i].GetTexture(1), m_Lights->GetDirection(), m_Lights->GetDiffuseColor()))
		{
			return false;
		}
	}
