...
void CameraClass::SetMouseSens(float mouseSens)
{
...
float CameraClass::GetMouseSens() const
{
...
void CameraClass::UpdateRotation(float offsetx, float offsety)
{
	m_rotationY += offsetx * m_mouseSensitivity;
	m_rotationX += offsety * m_mouseSensitivity;
...
void CameraClass::UpdatePosition(InputClass* Input, float frameTime)
{
	if (Input->IsKeyPressed('W'))
		Render(FORWARD, frameTime);
	if (Input->IsKeyPressed('A'))
		Render(LEFT, frameTime);
	if (Input->IsKeyPressed('S'))
		Render(BACKWARD, frameTime);
	if (Input->IsKeyPressed('D'))
		Render(RIGHT, frameTime);
	if (Input->IsKeyPressed('E'))
		Render(UP, frameTime);
	if (Input->IsKeyPressed('Q'))
		Render(DOWN, frameTime);
}
...
void CameraClass::Render(Camera_Movement direction, float deltaTime) {
	...
	XMFLOAT3 o = XMFLOAT3(0.0, 0.0, 0.0);
	...
	m_viewMatrix = XMMatrixLookAtLH(positionVector, origin, worldUpVector);
}
