...
bool ModelClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, char* modelFilename, char* textureFilename1)
{
	...
	if (!LoadTextures(device, deviceContext, textureFilename1)) return false;
	return true;
}
...
bool ModelClass::LoadTextures(ID3D11Device* device, ID3D11DeviceContext* deviceContext, char* filename1)
{
	// Create and initialize the texture object.
	m_Texture = new TextureClass;

	if (!m_Texture->Initialize(device, deviceContext, filename1)) return false;
