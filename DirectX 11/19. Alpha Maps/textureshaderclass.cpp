...
bool TextureShaderClass::Initialize(ID3D11Device* device, HWND hwnd) {
	...
	error = wcscpy_s(vsFilename, L"../DX11/texture.vs");
	if (error != 0) return false;

	error = wcscpy_s(psFilename, L"../DX11/texture.ps");
	if (error != 0) return false;
...
bool TextureShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename) {
	...
	// Buffers for shader bytecode
	ID3D10Blob* vertexShaderBuffer;
	ID3D10Blob* pixelShaderBuffer;

	// Structure of vertex buffer
	D3D11_INPUT_ELEMENT_DESC polygonLayout[2];
	unsigned int numElements;

	// Structure of matrix buffer
	D3D11_BUFFER_DESC matrixBufferDesc;

	// Desctription of texture sampler
	...
	// Set up the sampler state disc, the most important element is hte filter
	// Then it will be passed into the pixel shader 
