...
bool ApplicationClass::Initialize(int screenWidth, int screenHeight, HWND hwnd) {
	...
	// Create translate shader class
	m_TranslateShaderClass = new TranslateShaderClass;
	if(!m_TranslateShaderClass->Initialize(m_Direct3D->GetDevice(), hwnd, (WCHAR*)L"../DX11/translate.vs", (WCHAR*)L"../DX11/translate.ps", "TranslateVertexShader", "TranslatePixelShader"))
		return false;
...
void ApplicationClass::Shutdown()
{
	...
	if (m_TranslateShaderClass)
	{
		m_TranslateShaderClass->Shutdown();
		delete m_TranslateShaderClass;
		m_TranslateShaderClass = 0;
	}
...
bool ApplicationClass::Render()
{
	...
	m_TranslateShaderClass->m_translate = translate;
	m_TranslateShaderClass->m_texture = m_Model->GetTexture();

	m_Model->Render(m_Direct3D->GetDeviceContext());

	if (!m_TranslateShaderClass->Render(m_Direct3D->GetDeviceContext(), m_Model->GetIndexCount(), worldMatrix, viewMatrix, projectionMatrix))
		return false;
