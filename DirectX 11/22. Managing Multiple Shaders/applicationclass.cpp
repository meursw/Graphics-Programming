...
bool ApplicationClass::Initialize(int screenWidth, int screenHeight, HWND hwnd) {
	...
	m_ShaderManager->InsertShader(new NormalMapShaderClass);
	m_ShaderManager->InsertShader(new SpecMapShaderClass);
...
bool ApplicationClass::Render()
{
	...
	bool result;
	rotation -= 0.0174532925f * 0.0065f;
	if (rotation <= 0.0f)
	{
		rotation += 360.0f;
	}
	...
	for (ShaderClass* Shader : m_ShaderManager->GetShaders())
	{

		SpecMapShaderClass* SpecMapShader = dynamic_cast<SpecMapShaderClass*>(Shader);
		NormalMapShaderClass* NormalMapShader = dynamic_cast<NormalMapShaderClass*>(Shader);
		if (NormalMapShader != NULL)
		{
			NormalMapShader->m_colorTexture = m_Model[0].GetTexture(0);
			NormalMapShader->m_normalMapTexture = m_Model[0].GetTexture(1);

			NormalMapShader->m_lightDirection = m_Lights->GetDirection();
			NormalMapShader->m_diffuseColor = m_Lights->GetDiffuseColor();
		}

		if (SpecMapShader != NULL)
		{
			SpecMapShader->m_colorTexture = m_Model[1].GetTexture(0);
			SpecMapShader->m_normalMapTexture = m_Model[1].GetTexture(1);
			SpecMapShader->m_specMapTexture = m_Model[1].GetTexture(2);

			SpecMapShader->m_diffuseColor = m_Lights->GetDiffuseColor();
			SpecMapShader->m_specularColor = m_Lights->GetSpecularColor();
			SpecMapShader->m_specularPower = m_Lights->GetSpecularPower();
			SpecMapShader->m_lightDirection = m_Lights->GetDirection();
		
			SpecMapShader->m_cameraPosition = m_Camera->GetPosition();
		}
	}
	...
	for (int i = 0; i < 2; i++) {
		...
		if (!m_ShaderManager->RenderShaders(m_Direct3D->GetDeviceContext(), m_Model[i].GetIndexCount(), worldMatrix, viewMatrix, projectionMatrix, i))
			return false;
	}
